...
def _build_tool_research_fallback(
    *,
    ...
    wechat_summary = _compact(
        f"这是一份给完全不了解 {tool_name} 的新手看的深度实测教程：先判断它适不适合你，再看安装前检查、第一次怎么用、进阶场景和常见风险。",
        150,
    )
...
如果你完全不了解 {tool_name}，不要先急着下载安装。先判断它能不能解决你手里那个具体问题：写代码、整理资料、理解项目、生成内容，或者减少某个重复动作。

我会按“是什么、适合谁、安装前检查、第一次怎么用、进阶用法、常见坑、同类工具怎么比”这个顺序拆。资料里没明确写出的部分，我会标成“需要核验”，不替你编。
