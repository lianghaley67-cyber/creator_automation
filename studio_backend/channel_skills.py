...
我会先测 {tool_name}，不是为了追新工具，而是因为很多人真正卡住的是第一步：官网从哪进、装哪个版本、能不能读本地文件、会不会一上来就乱改。

所以我的判断标准很简单：它能不能帮我少掉一个重复动作，比如看懂项目、整理资料、写教程、拆步骤。第一步不是下载安装，而是先做这张检查表：
...
1. 看懂陌生内容：把项目、文章、资料先解释成人话。
2. 拆步骤：把“我要做什么”拆成第一步、第二步、检查点。
3. 做初稿：生成说明、清单、脚本、文档，但需要你复核。
4. 做复盘：记录哪里省时间、哪里不准，沉淀成自己的固定流程。
...
如果你也想试，可以先选一个最小任务：看懂一个项目、整理一段资料、写一篇初稿，或者拆一个自动化流程。你最想先省掉哪一步？
"""
    tags = []
...
- 如果使用“AI工具深度实测”类 Skill，公众号必须写成高密度教程文章：先给结论、工具是什么、核心信息卡、官方链接、逐步安装、10分钟实操、配置检查、发布前核验框、卡点排查、进阶使用、同类工具对比、最后结论。
- 工具教程开头不要长篇铺垫，不要连续制造焦虑钩子。开头 3 段内必须讲清：是否值得试、先检查什么、今天最小动作是什么。
- 工具教程开头不要重复标题摘要。标题下方摘要最多 1 句；“先给结论”必须用表格或清单给出判断标准，例如官方入口、系统支持、账号额度、文件权限。不能再用 2 段散文重复“不要急着安装”。
...
- 截图建议只保留真正辅助操作的内容，不要把截图清单写成独立长段落。图片说明必须能让读者知道“这一步该看哪里”。
...
{{
  "wechat": {{
    ...
    "markdown": "完整公众号文章（Markdown格式，1200-1800字，用##分节；工具教程必须包含：先给结论、工具是什么、核心信息卡、官方链接、逐步安装、10分钟实操、配置检查、发布前核验框、卡点排查、进阶使用、同类工具对比、最后结论。必须有1个短个人入口和1句中途踩坑经验，但不要写独立截图清单，不要重复钩子。最后结论必须短，2段以内，只留1个具体互动问题）"
  }},
  "xiaohongshu": {{
    "title": "小红书标题（不超过20字）",
