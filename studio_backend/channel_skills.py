...
{tool_name} 更适合先拿一个小任务试用，再决定要不要放进长期工作流。它的价值不在名字新不新，而在能不能帮你把写代码、整理资料、理解项目、生成内容这类任务变得更省力。

我更建议把它当成一个“先帮你拆步骤、再由你判断结果”的助手，而不是一个安装完就自动解决所有问题的工具。
...
1. 官网或官方下载入口。
2. 支持 Windows、macOS、Web 还是插件形式。
3. 是否需要账号、手机号、邮箱或海外网络环境。
4. 免费额度、付费规则和可用模型。
5. 是否会读取本地文件、项目代码或上传数据。
...
- 如果使用“AI工具深度实测”类 Skill，公众号必须写成深度教程文章：开头结论、工具是什么、核心信息卡、官方链接、逐步安装、10分钟实操、配置检查、卡点排查、基础使用、截图清单、进阶使用、问题避坑、同类工具对比、我的使用建议、最后结论。
...
{{
  "wechat": {{
    ...
    "markdown": "完整公众号文章（Markdown格式，1600-2600字，用##分节；工具教程必须包含：开头结论、工具是什么、核心信息卡、官方链接、逐步安装、10分钟实操、配置检查、卡点排查、截图清单、基础使用、进阶使用、问题避坑、同类工具对比、我的使用建议、最后结论）"
  }},
  "xiaohongshu": {{
    "title": "小红书标题（不超过20字）",
    "cover_text": "封面短句（不超过12字）",
    "body": "小红书正文（450-800字，必须包含：适合谁、安装前检查、10分钟实操任务、卡住排查、避坑提醒、可复制提示词）\\n\\n{tags_str}",
    "card_pages": [
