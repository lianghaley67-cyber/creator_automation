...
我会研究 {tool_name}，不是因为它又是一个新工具，而是因为很多人看到 AI 工具教程时，真正卡住的不是“知不知道名字”，而是不知道第一步到底点哪里。

比如：官网吗？下载哪个版本？安装后先点什么？会不会读取我的本地文件？这些问题没人讲清楚，新手就很容易收藏一堆教程，最后还是没跑起来。

所以这篇不按“功能介绍”写，而是按一个完全新手的视角，把 {tool_name} 从入口、安装、第一次测试，到哪些地方容易踩坑，完整跑一遍。

我的判断是：{tool_name} 更适合先拿一个小任务试用，再决定要不要放进长期工作流。它的价值不在名字新不新，而在能不能帮你把写代码、整理资料、理解项目、生成内容这类任务变得更省力。

## {tool_name} 是什么

{tool_name} 不是一个用来追热点的名字，而是一个需要放进具体场景里判断的 AI 工具。你可以先把它理解成：帮你把复杂任务拆成可执行步骤的助手。

如果它是编程类工具，重点看它能不能帮你理解项目、改代码、生成说明、排查错误。如果它是内容类工具，重点看它能不能帮你整理资料、生成脚本、提炼观点、做复盘。

## 核心信息卡

下面是当前资料里能看到的信息，建议你发布前再核验一次官方来源：
...
这一段建议你真的跟着做一次，不要只看介绍。

第一步：建一个安全测试文件夹。

你要做什么：在桌面新建一个文件夹，名字可以叫 `{tool_name}-test`。里面放一个 `README.md`，写三行普通内容：你现在想解决什么问题、你卡在哪里、你希望工具帮你输出什么。

为什么这样做：第一次不要拿真实项目、客户资料、账号信息测试，先用无风险内容观察工具反应。

成功标志：文件夹里只有一个测试文件，内容不涉及隐私。

失败处理：如果你一开始就想打开重要项目，先复制一份副本，不要直接操作原文件。

第二步：打开工具并导入测试内容。

你要做什么：打开 {tool_name}，优先找 New Project、Open Folder、Open Workspace、Chat、Ask AI 这类入口。把刚才的测试文件夹或测试文本放进去。

为什么这样做：你要先确认它能不能读懂你的上下文，而不是直接让它生成一大堆东西。

成功标志：界面里能看到你的测试文件、输入框或对话窗口。

失败处理：如果找不到入口，先看左侧菜单、顶部按钮、右下角聊天入口；仍然找不到，就回官方文档搜索 `quick start`、`open folder`、`new project`。

第三步：输入第一条提示词。

你要做什么：复制这段话：

“我是新手，请先不要修改文件。请用大白话解释这个测试文件夹里有什么、它适合解决什么问题、我下一步应该先做哪一个最小动作。请列出风险，不确定的地方标注需要核验。”

为什么这样做：第一条提示词的目的不是让它干活，而是看它能不能先解释清楚。

成功标志：输出里应该有三类内容：它看到的信息、建议步骤、风险提醒。

失败处理：如果它直接开始写代码或生成长文，就补一句：“先暂停，不要执行，只做解释和计划。”

第四步：只让它做一个小动作。

你要做什么：让它只改一处、只生成一个清单，或只整理一段内容。例如：“请把 README 改成 5 条检查清单，不要新增其他文件。”

为什么这样做：新手最容易犯的错，是第一次就让工具大改项目，最后不知道哪里变了。

成功标志：它的输出范围很小，你能看懂每一处变化。

失败处理：如果它一次改太多，撤回或不要保存，重新要求“只改一处”。

第五步：记录结果。

你要做什么：写下三句话：它帮我省了什么时间？哪里不准？下次提示词要怎么改？

为什么这样做：工具真正的价值不是“试过”，而是慢慢形成你自己的工作流。

成功标志：你得到一个可复用的小流程，而不是只收藏了一篇教程。

## 配置检查：想长期用，先看这几项

1. 账号和额度：到 Account、Billing、Plan 或 Usage 页面，看是否有免费额度、模型限制或付费提示。
2. 模型选择：到 Model、AI、Provider 或 Settings 页面，看当前默认模型是什么。
3. 文件权限：如果它要求读取本地文件夹，先确认是否只授权测试文件夹。
4. 自动修改：如果有 Auto Apply、Auto Save、Agent Mode 之类选项，新手建议先关闭或谨慎开启。
5. 语言偏好：如果有 Language、Response Language，可以设置为中文，工具名和按钮名保留英文即可。
6. 终端权限：如果它能运行命令，第一次不要让它自动执行安装、删除、发布类命令。

## 发布前再核验：这 5 件事别省

为了不误导读者，下面这些信息建议你发布前再打开官方页面确认一次：

1. 官方入口是不是 {official_line}，不要用第三方下载站替代。
2. 当前支持 Windows、macOS、Web、Linux 还是插件形式。
3. 是否必须登录账号，是否有地区、验证码或网络限制。
4. 免费额度、付费规则、可用模型有没有变化。
5. 是否会读取本地文件、项目代码或上传资料。

这部分集中核验就够了，不需要在文章每一段都提醒“资料不足”，否则读者会觉得整篇文章准备不充分。
...
1. 用它做资料整理：把一篇文章、一段文档或一组链接交给它，让它输出“是什么、适合谁、怎么用、风险点”。
2. 用它做项目理解：让它先解释结构，再建议第一步，不要直接让它大改。
3. 用它做内容复盘：让它把你的使用过程整理成“步骤、结果、问题、下一次怎么改”。
4. 用它做选题拆解：让它把一个工具拆成安装篇、入门篇、案例篇、对比篇、避坑篇。

## 卡住了怎么排查

1. 官网打不开：先确认链接是不是 {official_line}，再换网络或浏览器；不要立刻下载陌生安装包。
2. 下载按钮找不到：先找 Download、Get Started、Try、Sign in；有些工具会先让你登录再下载。
3. 登录失败：检查验证码、邮箱、手机号、地区限制、浏览器插件拦截。
4. 模型不可用：去 Settings、Model、Account 或 Usage 看是否需要选择模型、绑定账号或开通额度。
5. 文件权限弹窗：只授权测试文件夹，不要一次性授权整个桌面或私人资料目录。
6. 输出胡编：让它列证据来源，要求“不确定就说不知道”；涉及安装、价格、官方承诺时必须回官网核验。
7. 改动太大：要求它先给计划，再逐步执行；每次只允许改一个文件或生成一个清单。

## 同类工具怎么比

如果你要判断 {tool_name} 值不值得长期用，不要只看“功能多不多”，建议按这几个维度比较：

| 维度 | {tool_name} | 同类工具 |
| --- | --- | --- |
| 适合人群 | 资料不足，建议核验 | 看是否更适合新手/专业用户 |
| 上手难度 | 看安装、登录、配置是否顺畅 | 看是否需要复杂环境 |
| 核心能力 | 看它最擅长解决什么任务 | 不要只看宣传词 |
| 成本 | 核验免费额度和付费规则 | 对比长期使用成本 |
| 数据风险 | 看是否读取本地文件或上传资料 | 敏感资料谨慎使用 |
...
坑四：第一次就拿重要项目测试。新工具先用测试文件夹，不要直接处理重要资料。

坑五：只问“帮我写一下”。真正好用的问法是给背景、目标、限制和输出格式。

## 我的使用建议

如果你是完全新手：先用它完成一个小任务，不要追求全功能。

如果你是内容创作者：把它当资料整理和教程拆解助手，用完一定要补自己的判断。

如果你想把 AI 放进工作流：固定一个重复动作，比如整理资料、写选题、拆步骤，每次记录节省了多少时间。

## 最后结论

{tool_name} 可以先小范围试一下，但别一上来就把重要资料交给它。今天最小动作很简单：找官方入口，建一个测试文件夹或测试文档，只让它帮你完成一个小练习。

如果你也卡在第一步，可以告诉我：你现在最想让 AI 帮你省掉哪件事？看代码、整理资料、写文章，还是搭自动化流程？我下一篇就按这个场景继续拆。
"""
    tags = []
    ...
    xhs_body = "\n\n".join(
        [
            f"{tool_name} 到底能干什么？",
            f"我会研究 {tool_name}，不是为了追新工具，而是因为很多人真正卡住的是：官网在哪、点哪里下载、装完第一步怎么试。",
            ...
            "截图建议：\n1. 官网首页\n2. 下载按钮\n3. 登录/工作台页面\n4. 第一个测试任务页面",
            "发布前再核验：\n价格、系统支持、模型额度、本地文件权限、账号/地区限制，都以官网当前页面为准。",
...
- 如果使用“AI工具深度实测”类 Skill，公众号必须写成深度教程文章：我为什么测它、工具是什么、核心信息卡、官方链接、逐步安装、10分钟实操、配置检查、发布前核验框、卡点排查、基础使用、截图清单、进阶使用、问题避坑、同类工具对比、我的使用建议、最后结论。
- 工具教程开头必须像一个真实的人在替读者试路：写清“我为什么测它”，连接普通人的具体麻烦，例如官网入口、下载版本、安装后第一步、权限和付费焦虑。不要直接从产品定义开头。
...
- 必须把所有不确定信息集中成一个“发布前核验框”，不要在文章里到处重复“资料不足”。核验框至少包含：官方入口、价格/额度、系统支持、本地文件权限、账号/地区/网络限制。
- 必须给常见卡点排查：官网打不开、下载按钮找不到、登录失败、模型不可用、文件权限弹窗、输出胡编、改动太大。
- 截图不能只写“附图”，必须写成截图建议，例如：[截图建议：截官网首页，画面里要看到官方域名和 Download 按钮]。
- 最后结论必须短，控制在 2 段以内，每段 1-2 句。第一段说清是否值得试和今天最小动作；第二段只留 1 个具体互动问题。不要连续抛多个问题，不要写“欢迎评论”，要像真人问朋友，例如：如果你也卡在第一步，可以告诉我：你现在最想让 AI 帮你省掉哪件事？看代码、整理资料、写文章，还是搭自动化流程？
...
{{
  "wechat": {{
    ...
    "markdown": "完整公众号文章（Markdown格式，1600-2600字，用##分节；工具教程必须包含：我为什么测它、工具是什么、核心信息卡、官方链接、逐步安装、10分钟实操、配置检查、发布前核验框、卡点排查、截图清单、基础使用、进阶使用、问题避坑、同类工具对比、我的使用建议、最后结论。最后结论必须短，2段以内，只留1个具体互动问题）"
  }},
  "xiaohongshu": {{
    "title": "小红书标题（不超过20字）",
    "cover_text": "封面短句（不超过12字）",
    "body": "小红书正文（450-800字，必须包含：我为什么测它、适合谁、安装前检查、10分钟实操任务、发布前核验、卡住排查、避坑提醒、可复制提示词、具体互动问题）\\n\\n{tags_str}",
    "card_pages": [
