...
{tool_name} 值不值得试，关键不在它名字热不热，而在它能不能帮你完成一个真实任务。今天最小动作是：先找官方入口，建一个测试文件夹或测试文档，让它只完成一个小练习。

你可以继续追问：{tool_name} 和同类工具有什么区别？它适不适合我的工作流？有没有适合小白的第一个练习任务？如果要做成课程或小红书系列，应该拆成哪几篇？

如果你也经常卡在“工具看起来很好，但第一步不知道怎么开始”，可以告诉我你现在最想让 AI 帮你省掉哪一步：看代码、整理资料、写文章，还是搭自动化流程？我下一篇可以按你的场景继续拆。
"""
    tags = []
    ...
    xhs_body = "\n\n".join(
        [
            ...
            "你现在最想让 AI 帮你省掉哪一步：看代码、整理资料、写文章，还是搭自动化流程？",
...
- 结尾必须给一个具体互动问题，引导后续选题。不要写“欢迎评论”，要写成读者能回答的选择题，例如：你现在最想让 AI 帮你省掉哪一步：看代码、整理资料、写文章，还是搭自动化流程？
...
{{
  "wechat": {{
    ...
    "markdown": "完整公众号文章（Markdown格式，1600-2600字，用##分节；工具教程必须包含：我为什么测它、工具是什么、核心信息卡、官方链接、逐步安装、10分钟实操、配置检查、发布前核验框、卡点排查、截图清单、基础使用、进阶使用、问题避坑、同类工具对比、我的使用建议、最后结论、具体互动问题）"
  }},
  "xiaohongshu": {{
    "title": "小红书标题（不超过20字）",
