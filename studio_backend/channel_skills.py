...
我会先测 {tool_name}，是因为我在做“看懂项目、整理资料、写教程”这类重复工作时，最怕两件事：入口找错、权限给大，还没跑通就把真实资料丢进去。

所以我不先介绍功能，而是先看它能不能帮我省掉一个具体动作：把陌生内容解释清楚、拆出下一步，并提醒哪些地方需要我自己核验。第一步不是下载安装，而是先做这张检查表：

| 先查什么 | 怎么判断 |
| --- | --- |
| 官方入口 | 只从官网或官方文档进，不用网盘包、论坛包、陌生下载站 |
| 系统支持 | 看清支持 Windows、macOS、Linux、Web 还是插件 |
| 账号额度 | 找 Account / Plan / Billing / Usage，确认是否需要登录、是否有免费额度 |
| 文件权限 | 如果要打开本地文件夹，只授权测试文件夹，不给桌面和隐私目录 |

通过这 4 项后，再建一个 `{tool_name}-test` 测试文件夹，放一份普通文档，让它先解释、列计划、指出风险。能跑通这个最小任务，再考虑放进真实工作流。

## {tool_name} 是什么

{tool_name} 可以先理解成一个 AI 工作助手。判断它有没有用，不看宣传词，看它能不能帮你完成一个具体任务：理解项目、整理资料、生成说明、排查错误、拆解步骤或复盘结果。

## 安装前先检查

先不要急着点下载，先把这几件事确认完：

1. 官方入口：{official_line}
2. 安装来源：优先从官方链接进入，不要从网盘、陌生论坛或不明安装包下载。
3. 账号要求：资料不足，建议以官方页面提示为准。
4. 费用/额度：资料不足，建议登录后查看官方说明。
5. 数据权限：如果要打开本地文件、项目代码或上传资料，先确认权限和隐私提示。

## 它的功能，对普通人意味着什么

1. 看懂陌生内容：不是让你记住一个新工具名，而是把项目、文章、资料先翻译成人话，让你知道下一步该看哪里。
2. 拆步骤：不是替你做决定，而是把“我要做什么”拆成第一步、第二步、检查点，减少你卡在开头的时间。
3. 做初稿：不是直接交付成品，而是先生成说明、清单、脚本或文档草稿，你再负责判断和删改。
4. 做复盘：不是简单总结，而是记录哪里省时间、哪里不准、下次提示词怎么改，慢慢沉淀成自己的固定流程。

## 适合谁，不适合谁

适合：想用 AI 辅助写代码、整理资料、写教程、理解项目的人；也适合想把重复动作慢慢做成流程的人。

不适合：期待装完就自动解决所有问题的人；不愿意检查权限、账号、费用和数据上传风险的人。
...
1. 建测试文件夹：桌面新建 `{tool_name}-test`，里面只放一个 `README.md`。写三行：我想解决什么问题、我卡在哪里、我希望输出什么。成功标志：不含客户资料、账号、私人文件。

2. 打开工具导入测试内容：找 New Project、Open Folder、Open Workspace、Chat、Ask AI 这类入口。成功标志：界面里能看到你的测试文件、输入框或对话窗口。

3. 输入第一条提示词：

“我是新手，请先不要修改文件。请用大白话解释这个测试文件夹里有什么、下一步应该先做哪个最小动作。请列出风险，不确定的地方标注需要核验。”

成功标志：输出里有“它看到的信息、建议步骤、风险提醒”。如果它直接开始写代码，就补一句：“先暂停，不要执行，只做解释和计划。”

我自己第一次测试这类工具时，最容易犯的错就是一上来让它处理太多内容，最后看不懂它到底改了哪里。现在我会先让它解释，再让它只做一个小动作。

4. 只让它做一个小动作：比如“把 README 改成 5 条检查清单，不要新增其他文件”。成功标志：改动范围很小，你能看懂每一处变化。

5. 写复盘：记录三句话：它帮我省了什么时间？哪里不准？下次提示词怎么改？这一步决定它能不能变成你的长期流程。

[截图建议：截第一次提示词和输出结果，画面里要看到“解释、计划、风险提醒”，方便读者确认自己有没有跑对。]
...
1. 官网入口图：用来确认域名和下载入口，避免读者点到第三方安装包。
2. 下载/开始使用按钮图：用来确认第一步该点 Download、Get Started、Try 还是 Sign in。
3. 登录或工作台首页图：用来确认安装后是否真的进入了可操作界面。
4. 第一个测试任务图：用来确认工具有没有先解释、列计划、提醒风险，而不是直接乱改。
5. 结果复盘图：用来确认这次测试到底省了哪一步、哪里还需要人工判断。
...
{tool_name} 可以试，但不要直接拿真实项目试。今天只做一件事：用测试文件夹跑完“解释、列计划、做一个小改动、复盘”。
...
- 如果使用“AI工具深度实测”类 Skill，公众号必须写成高密度教程文章：开头直接给判断、工具是什么、安装前检查、官方链接、逐步安装、10分钟实操、配置检查、发布前核验框、截图清单、卡点排查、进阶使用、同类工具对比、最后结论。不要单独写信息汇总小节，把官方入口、账号、费用、权限并入安装前检查和发布前核验。
...
- 工具教程必须保留真实使用入口，但只能短写：开头 3 段内放 1-2 句第一人称，讲清“我为什么测它”，必须连接一个具体麻烦，例如整理资料太慢、看不懂项目、安装入口混乱、怕误授权本地文件。不能写成励志开场或连续钩子。
...
{{
  "wechat": {{
    ...
    "markdown": "完整公众号文章（Markdown格式，1200-1800字，用##分节；工具教程必须包含：开头直接给判断、工具是什么、安装前检查、官方链接、逐步安装、10分钟实操、配置检查、发布前核验框、截图清单、卡点排查、进阶使用、同类工具对比、最后结论。禁止写##先给结论或##先说结论，标题和摘要后直接进入正文判断。禁止单独写信息汇总小节，把官方入口、账号、费用、权限并入安装前检查和发布前核验。正文第一段必须从我的具体场景开始，功能说明必须翻译成对普通人意味着什么。必须有1个短个人入口和1句中途踩坑经验，截图清单只保留操作判断需要的截图，不要重复钩子。最后结论必须短，2段以内，只留1个具体可回答的选项式互动问题）"
  }},
  "xiaohongshu": {{
    "title": "小红书标题（不超过20字）",
    "cover_text": "封面短句（不超过12字）",
    "body": "小红书正文（450-800字，必须包含：我为什么测它、适合谁、安装前检查、10分钟实操任务、截图建议、发布前核验、卡住排查、避坑提醒、可复制提示词、具体互动问题。开头从我的具体场景开始，功能必须说成人话，结尾必须是可回答的选项题）\\n\\n{tags_str}",
    "card_pages": [
