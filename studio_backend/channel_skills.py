...
CHANNEL_SKILLS: dict[str, dict[str, Any]] = {
    ...
    "wechat_tool_deep_review_v1": {
        ...
        "file": "wechat/tool_deep_review_wechat.md",
        "description": "五幕叙事结构：我为什么测它 → 下载前的判断框架 → 第一次完整上手实录 → 我真正在用它做什么 → 给同路人的选择建议。不是产品说明书，是学习旅程陪伴。",
        "persona_tags": ["工具实测", "深度教程", "学习旅程", "零基础上手"],
        "example": {
            "title": "用了 Claude Code 两周之后，我才搞明白它真正有用的地方",
            "summary": "开头写的不是工具介绍，而是：我当时在整理别人的项目，卡在不知道第一步点哪里，然后有人推荐了它。读者跟着我走一遍从陌生到上手的路。",
            "excerpt": "上周我在整理一个别人给我的项目，二十几个文件，完全不知道该从哪里下手……\n\n## 在你下载之前，先做这个判断\n\n【适合你，如果你……】\n- 经常需要理解别人写的项目或文档\n- 想让 AI 帮你改代码但不敢乱动真实文件\n\n【下载前先确认】\n- 官方入口：需核验\n- 费用：有免费额度，具体以官网为准\n\n## 我的第一次完整上手记录\n\n第二步我卡了一下——登录后找不到新建项目的入口，翻了两分钟才发现按钮在右上角……",
        },
    },
...
def _skill_type_contract(skill_id: str, channel: str) -> str:
    ...
    if kind == "fiction_serial":
        return (
            ...
            "- 【引流禁止·最高优先级】正文中绝对不能出现：http/https网址、微信号/QQ号/手机号/邮箱、"
            "平台名称+账号（如'关注我的微博'）、任何引导读者离开番茄的信息"
        )
    if kind == "ai_tips_discovery":
        return (
    ...
    if kind == "tool_deep_review":
        return (
            f"【{channel} 当前 Skill 类型：AI工具深度实测·五幕叙事】\n"
            "- 这是学习旅程陪伴文，不是产品手册，禁止写成检查清单或安装步骤表格。\n"
            "- 全篇必须有'我'的声音：我为什么测它、我卡在哪里、我真正在用它做什么。\n"
            "- 开头第一段必须从'我当时在做什么/卡在哪里'进入，禁止从工具介绍或功能列表开始。\n"
            "- 每篇只讲一个核心发现，读者读完要能做一个决定并带走一个判断框架。\n"
            "- 结构遵循五幕：为什么测它 → 下载前判断 → 第一次上手实录 → 我真正在用它做什么 → 给同路人的建议。\n"
            "- 禁止写成：安装前检查表格、逐步命令行步骤框、颜色标注的步骤卡片、产品截图清单。"
        )
    if kind == "tool_research":
        return (
            f"【{channel} 当前 Skill 类型：工具研究说明书】\n"
...
def _fanqie_content_review(
    ...
    *,
    ...
    _url_pattern = _re_check.compile(r'https?://\S+|www\.\S+', _re_check.IGNORECASE)
    _contact_pattern = _re_check.compile(
        r'微信[号：:]\s*\S+|QQ[号：:]\s*\d+|1[3-9]\d{9}|[\w.+-]+@[\w-]+\.\w+'
    )
    _quick_issues = []
    if _url_pattern.search(content):
        _quick_issues.append("正文包含外部网址（http/https链接），番茄平台直接驳回")
    if _contact_pattern.search(content):
        _quick_issues.append("正文包含联系方式（微信/QQ/手机号/邮箱），番茄平台直接驳回")
    if _quick_issues:
        return {
            "pass": False,
            "risk_level": "high",
            "issues": _quick_issues,
            "suggestions": ["删除正文中所有外部链接和联系方式，改写相关段落"],
        }

    prompt = f"""你是番茄小说内容审核员。审核以下小说章节是否符合平台规范，直接输出JSON，不加任何说明。

【审核规范 · 言情玄幻类重点条目】
1. 推广引流（最高优先级）：正文含外部网址、联系方式（微信/QQ/手机/邮箱）、引导至其他平台的信息 → 直接驳回
2. 色情/低俗：禁止大篇幅性行为细节描写；禁止乱伦、未成年性行为、强奸详细描述；禁止对角色低俗意淫
3. 未成年负面导向：禁止未成年人性行为/性暗示；禁止宣扬霸凌、早恋（未成年）、吸烟斗殴
4. 血腥引人不适：禁止大篇幅血腥细节特写（如眼球崩裂、肠子外露等极端描写）
5. 违背公序良俗：主角/主线不得宣扬错误价值观；不得恶意挑拨性别/阶级对立
6. 标题：不含敏感词，与正文内容相符
7. 格式：段落正常分隔，无全繁体/乱码/重复内容
...
- 公众号字数控制在1200-1800字。"""
    ...
    # ── AI工具深度实测·五幕叙事规则 ─────────────────────────
    tool_deep_review_rules = """AI工具深度实测·五幕叙事规则类型：

【写作本质】：这是学习旅程陪伴文，不是产品手册。
读者读完必须得到三样东西：
① 一个决定（这个工具值不值得我花时间试）
② 一个框架（这类工具我以后怎么判断）
③ 一个今天可以做的具体动作

【五幕结构——严格按此输出，禁止替换为其他结构】：

第一幕（150-200字）：我为什么在乎这个
- 第一句必须是：我当时在做什么（具体动作，如"整理别人给我的项目""带娃间隙想写个脚本"）
- 第二句：我遇到的具体障碍（不是"效率低"，而是"不知道从哪个文件开始""不敢让AI直接改代码"）
- 第三句：我为什么选择试这个工具（来自真实推荐或具体理由，不是"它很热门"）
- 禁止用工具功能介绍或市场背景当开头

第二幕（200-300字）：在你下载之前，3分钟做这个判断
- 格式：适合你，如果你……（3个具体场景）/ 暂时不适合，如果你……（具体限制）
- 必须包含：官方入口、系统要求、费用、是否需要账号、是否读本地文件（信息不确定写"建议核验"）
- 必须有一句话说明这个工具解决不了什么（诚实边界）

第三幕（400-600字）：我的第一次完整上手实录
- 按时间顺序，每步写：做什么 → 你会看到什么 → 如果卡住先查哪里
- 必须有至少1个我实际卡住的地方，写清楚卡在哪、怎么解决
- 必须有1句"到这里它开始有用了"的时刻
- 给1段可直接复制的第一次提示词
- 每3步插1句感受，不能全是步骤清单
- 禁止写：命令行步骤框、颜色标注的卡片、表格清单、"截图X"系列

第四幕（200-300字）：用了一段时间，我真正在用它做什么
- 格式：我固定在用的2-3件事（场景+具体用法+省了哪一步）
- 必须有：我试过但放弃的1个用法 + 原因（比"局限性"更可信）
- 说清这个工具在我流程里的位置

第五幕（150-200字）：给同路人的判断框架
- 三类人分别给一句具体建议：完全新手 / 已在用AI想切换的 / 时间很少只想判断值不值得
- 结尾只留1个选择题（A/B/C/D格式），读者不用思考太久就能回答
- 这道题要能引导下一篇选题

【标题格式优先选】：
- "用了[工具名]两周之后，我才搞明白它真正有用的地方"
- "在你下载[工具名]之前，我帮你踩了这些坑"
- "[工具名]零基础上手：我卡了一周，整理给你看"
禁止："[工具名]完全使用指南" / "[工具名]超强功能大全" / "[工具名]安装配置教程"

【声音放大器——每篇至少用3句，让内容有真实感】：
...
- 【引流禁止·最高优先级】正文中绝对不能出现：任何http/https网址、微信号/QQ号/手机号/邮箱、引导读者去其他平台的任何信息
