...
def _call_online_chapter_generation(
    ...
    *,
    ...
    system_prompt = (
        ...
        "删除所有不推动剧情的句子；每段必须有变化，不能写可删段落。"
        "冲突必须升级，不允许同一压力换说法重复；情绪必须递进，不能停留在恐惧、震惊或犹豫。"
        "结尾必须制造强悬念：新危险、新反转、新证据或角色必须立刻回应的问题。"
        "如果内容可删减30%以上、出现同义重复或无实质推进，本章会被判定失败并整章重写。"
    ...
    if rewrite_feedback:
        feedback_text = (
            ...
            + "\n硬性处理：删除回忆段、解释段、同义重复段和所有不推动剧情的句子；每一段都必须有当前场景动作、对话、线索、冲突升级或情绪递进；结尾必须换成更强悬念。"
        )
